...
from .indexbuilder import rebuild_index

lastmodifiedFile = "None"
...
class H5EventHandler(FileSystemEventHandler):
    ...
    def on_modified(self, event):
        ...
        what = 'directory' if event.is_directory else 'file'
        if not op.isdir(event.src_path) and event.src_path != "/data/.toc.h5" and str(event.src_path).endswith(".h5"):
            self.log.info("H5EventHandler -- Modified %s: %s was updated", what, event.src_path)
