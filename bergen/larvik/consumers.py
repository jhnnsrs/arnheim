...
class Larvik2Consumer(AsyncConsumer):
    ...
    async def startJob(self, data):
        ...
        try:
            ...
            for modelname, modelparams in returndict.items():
                createModelList = await self.getModelFuncDict()[modelname](modelparams,self.request,self.settings)
                for item in createModelList:
                    await self.modelCreated(item[0], self.getSerializerDict()[item[1]], item[3]) # Expects the function to return a list of Items with there corresponding Serizliaer
                    self.logger.info(item[3] + " Model " + item[1])
